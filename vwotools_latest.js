...
function modifyTable() {
	...
	rows.each(function () {
		...
		if (!isControl && !isDisabled) {//skip Control row and identically performing rows
			...
			var samplesizeHTML;
			var title = "If the true effect is " + pctEffect + "%, you should test with " + samplesizeDifference + " more visitors than you have now (" + Math.ceil(elapsedWeeks - Math.floor(elapsedWeeks) + (samplesize-visitors)/(trafficWeekly/rowCountActive)) + " weeks). Then, you would have a 5%+ chance of seeing a similar result by chance (false positive) and 15%+ chance of seeing a real effect as inconclusive (false negative).";
